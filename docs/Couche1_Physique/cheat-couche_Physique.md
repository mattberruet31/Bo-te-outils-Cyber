...
**Cheat-couche_Physique.md** (à placer dans `docs/Couche1_Physique/`)
...
- Proposer les commandes associées pour chaque attaque (commandes présentées dans des blocs de code).
...
Saturer une bande de fréquence en émettant un signal bruité afin de perturber les communications.
...
Le script automatisé analysera ce fichier pour :
1. Lister les outils disponibles (lignes commençant par **Outil:**).
2. Afficher, pour l’outil sélectionné, les attaques disponibles (lignes commençant par **Attaque:**).
3. Proposer les commandes associées pour l’attaque choisie (commandes présentées dans des blocs de code sous **Commandes Clés**).
